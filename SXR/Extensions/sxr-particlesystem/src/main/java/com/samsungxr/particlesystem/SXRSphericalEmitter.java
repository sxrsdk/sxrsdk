...
public class SXRSphericalEmitter extends SXREmitter{
    ...
    private SXRDrawFrameListenerImpl mFrameListener;

    public SXRSphericalEmitter(SXRContext gvrContext) {
        super(gvrContext);
        mFrameListener = new SXRDrawFrameListenerImpl(this);
        mSXRContext.registerDrawFrameListener(mFrameListener);
    }
    ...
    private static final class SXRDrawFrameListenerImpl implements SXRDrawFrameListener {
        ...
        private final WeakReference<SXRSphericalEmitter> mRef;
        private float totalTime = 0;
        private float mElapsedTime = 0;
        SXRDrawFrameListenerImpl(final SXRSphericalEmitter emitter) {
            mRef = new WeakReference<SXRSphericalEmitter>(emitter);
        }
        ...
        public void onDrawFrame(float frameTime) {
            ...
            final SXRSphericalEmitter emitter = mRef.get();
            if (null != emitter)
            {
                totalTime += frameTime;
    ...
    {
        mSXRContext.unregisterDrawFrameListener(mFrameListener);
        super.clearSystem();
    }
}
