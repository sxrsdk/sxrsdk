...
public class SXRConsole extends SXRMaterial
{
    ...
    public static class ConsoleShader extends SXRShaderTemplate
    {
        ...
        public ConsoleShader(SXRContext ctx)
        {
     ...
     * value to {@linkplain SXRConsole#SXRConsole(SXRContext, EyeMode) the
     * constructor} but can change it <i>via</i>
     * {@link SXRConsole#setEyeMode(EyeMode)}
     */
    public enum EyeMode {
         ...
         * {@link SXRConsole#writeLine(String, Object...) writeLine()}, but that
    ...
    private SXRTexture texture = null;
     ...
     */
    public SXRConsole(SXRContext gvrContext, EyeMode startMode) {
        this(gvrContext, startMode, gvrContext.getMainScene());
    }

    /**
     * Create a console, specifying the initial eye mode and the
     * {@link SXRScene} to attach it to.
     *
     * This overload is useful when you are using
     * {@link SXRContext#getMainScene()} and creating your debug console in
     * {@link com.samsungxr.SXRMain#onInit(SXRContext)}.
     *
     * @param gvrContext
     *            The SXR context.
     * @param startMode
     ...
     *            {@link #setEyeMode(EyeMode)}
     * @param gvrScene
     *            The {@link SXRScene} to attach the console to; this is useful
     *            when you want to attach the console to the
     *            {@linkplain SXRContext#getMainScene() next main scene.}
     */
    public SXRConsole(SXRContext gvrContext, EyeMode startMode,
                      SXRScene gvrScene) {
     ...
     * {@linkplain #SXRConsole(SXRContext, EyeMode) the constructor,} but you
    ...
    public void setEyeMode(EyeMode newMode) {
        setEyeMode(newMode, getSXRContext().getMainScene().getMainCameraRig());
    }

    private void setEyeMode(EyeMode newMode, SXRCameraRig cameraRig) {
        eyeMode = newMode;
    ...
    private void setMainTexture() {
        ...
        if (texture == null)
        {
            texture = new SXRTexture(getSXRContext());
        }
        SXRImage image = texture.getImage();
        if (image != null)
        {
            if (SXRBitmapImage.class.isAssignableFrom(image.getClass()))
            {
                SXRBitmapImage bmapImage = (SXRBitmapImage) image;
        ...
        if (!textureUpdated)
        {
            image = new SXRBitmapImage(getSXRContext(), HUD);
            texture.setImage(image);
            setTexture("u_overlay", texture);
        }
    }

    private static synchronized SXRShaderId getShaderId(SXRContext gvrContext) {
        if (shaderId == null)
        {
