...
/**
 * Captures rendered contents from a shader into a {@code SXRRenderTexture} while
 ...
 */
public class SXRTextureCapturer extends SXRHybridObject {
    private static final String TAG = SXRTextureCapturer.class.getSimpleName();
    ...
    protected SXRRenderTexture captureTexture;
     ...
     */
    public SXRTextureCapturer(SXRContext gvrContext, int width, int height, int sampleCount) {
    ...
    public void update(int width, int height, int sampleCount) {
        ...
        if (sampleCount == 0)
            captureTexture = new SXRRenderTexture(getSXRContext(), width, height);
        else
            captureTexture = new SXRRenderTexture(getSXRContext(), width, height, sampleCount);

        setRenderTexture(captureTexture);
        readBackBuffer = new int[width * height];
    }

    private void setRenderTexture(SXRRenderTexture screenshot) {
    ...
    protected void callbackFromNative(int index, String info) {
        switch (index) {
            case TCCB_NEW_CAPTURE:
                ...
                getSXRContext().runOnGlThreadPostRender(1 /* delay 1 frame */, new Runnable() {
                    @Override
                    public void run()
                    {
...
class NativeTextureCapturer {
    ...
    static native void setCapturerObject(long capturer, SXRTextureCapturer capturerObject);
