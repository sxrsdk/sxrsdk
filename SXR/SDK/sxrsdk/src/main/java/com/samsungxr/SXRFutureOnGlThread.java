 ...
 * Thread.start(SXRFutureOnGlThread).
 * 
 * The most common use cases should be with SXRContext.runOnGlThread. You can
 * submit the call request through SXRContext.runOnGlThread(SXRFutureOnGlThread)
 * and get the result with get() method. Just be careful that it's best to call
 * SXRContext.runOnGlThread(SXRFutureOnGlThread) when
 * SXRContext.isCurrentThreadGLThread() returns false since you can directly
 * call SXRFutureOnGlThread.run() while in GL thread.
 * 
 */
...
public class SXRFutureOnGlThread<T> implements RunnableFuture<T> {
     ...
     * SXRContext.runOnGlThread(). This will be automatically called in GL
     ...
     * which is running the request.But SXRFutureOnGlThread will probably run on
        ...
        synchronized (lock) {
            if (mIsCancelled) {
                throw new CancellationException(
                        "The task on SXRFutureOnGlThread has already been cancelled.");
            }
            if (mIsDone) {
                return t;
            }
