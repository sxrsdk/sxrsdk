 ...
 * Controller/Gamepad to manipulate {@link SXRGamepadController}s.
 */
class SXRGamepadDeviceManager {
    private static final String TAG = SXRGamepadDeviceManager.class
            .getSimpleName();
    ...
    private SparseArray<SXRGamepadController> controllers;
    ...
    /**
     * {@link SXRGamepadDeviceManager} is a helper class that can be used to
     ...
     * performing SXRf actions. <br>
     *
     * 2) Provide x, y, and z displacement values to the app. <br>
     *
     * 3) Create a new {@link SXRGamepadController} object whenever a new
     * controller/gamepad device is detected by the {@link SXRInputManager}.
     *
     */
    SXRGamepadDeviceManager() {
    ...
    private static class SXRGamepadController extends SXRCursorController {
    ...
    private class EventHandlerThread extends Thread {
                ...
                while (true) {
                    synchronized (lock) {
                        ...
                        for (int i = 0; i < holders.size(); i++) {
                            ...
                            SXRGamepadController controller = controllers.get(id);
         ...
         */
        private void dispatchKeyEvent(SXRGamepadController controller,
                                      KeyEvent event) {
        ...
        private void dispatchMotionEvent(SXRGamepadController controller,
                                         MotionEvent event) {
            ...
            if (vendorId == SXRDeviceConstants.SAMSUNG_GAMEPAD_VENDOR_ID
                    && productId == SXRDeviceConstants.SAMSUNG_GAMEPAD_PRODUCT_ID) {
                ry = getCenteredAxis(event, device, MotionEvent.AXIS_RY);
            } else if ((vendorId == SXRDeviceConstants.SONY_DUALSHOCK_CONTROLLER_VENDOR_ID
                    && (productId == SXRDeviceConstants.SONY_DUALSHOCK_3_CONTROLLER_PRODUCT_ID
                    || productId == SXRDeviceConstants.SONY_DUALSHOCK_4_CONTROLLER_PRODUCT_ID))) {
                ry = getCenteredAxis(event, device, MotionEvent.AXIS_RZ);
            } else if ((vendorId == SXRDeviceConstants.STEELSERIES_CONTROLLER_VENDOR_ID
                    && productId == SXRDeviceConstants.STEELSERIES_CONTROLLER_PRODUCT_ID)) {
                ry = getCenteredAxis(event, device, MotionEvent.AXIS_RZ);
