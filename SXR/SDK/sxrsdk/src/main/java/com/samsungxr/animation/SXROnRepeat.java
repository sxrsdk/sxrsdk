...
public interface SXROnRepeat extends SXROnFinish {
     ...
     * a desired state.
     * 
     * When you {@linkplain SXRAnimation#setOnFinish(SXROnFinish) set} a
     * {@code SXROnRepeat} callback, {@code iteration} will be called after each
     * iteration: returning {@code false} will stop the animation.
     * 
     * <p>
     * The repeat count {@linkplain SXRAnimation#DEFAULT_REPEAT_COUNT defaults
     * to 2,} but setting a {@code SXROnRepeat} callback also sets the repeat
     * count to a negative number, so that the animation is under your
     * callback's control. Do note that {@link SXRAnimation#setRepeatCount(int)
     * setting a positive repeat count} <em>after</em> setting the
     * {@code SXROnRepeat} callback will override this: {@code iteration} is not
     ...
     *         {@linkplain SXRAnimationEngine#stop(SXRAnimation) unregister the
     *         animation} <em>and</em> call
     *         {@link SXROnFinish#finished(SXRAnimation)}.
     */
    boolean iteration(SXRAnimation animation, int count);
}