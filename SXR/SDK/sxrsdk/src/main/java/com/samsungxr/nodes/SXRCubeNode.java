...
public class SXRCubeNode extends SXRNode {
     ...
     * {@code SXRMaterial.SXRShaderType.Texture} and a {@code SXRTexture}.
     *
     * To use different textures on different faces, use a material
     * with the shader type {@code SXRMaterial.SXRShaderType.Cubemap}, and a
     * cubemap texture loaded by {@code SXRContext.loadCubemapTexture}.
     * 
     * @param gvrContext
     *            current {@link SXRContext}
     * 
     ...
     *            {@code SXRTexture} is used here for asynchronously
     ...
     * 
     * @param textureList
     *            the list of six textures for six faces. {@code tSXRTexture} is used here for asynchronously loading
    ...
    public static SXRMesh createCube(SXRContext gvrContext, String descriptor, boolean facingOut, Vector3f dimensions)
    {
        SXRMesh mesh = new SXRMesh(gvrContext, descriptor);
    ...
    private void createSimpleCube(SXRContext gvrContext, boolean facingOut,
            SXRMaterial material, Vector3f dimensions) {

        SXRMesh mesh = createCube(gvrContext, "float3 a_position, float2 a_texcoord, float3 a_normal", facingOut, dimensions);
    ...
    private SXRMesh createMesh(SXRVertexBuffer vbuf, char[] indices)
    {
        final SXRIndexBuffer ibuf = new SXRIndexBuffer(vbuf.getSXRContext(), 2, 6);
        ibuf.setShortVec(indices);
        return new SXRMesh(vbuf, ibuf);
    }
    ...
    private void createComplexCube(SXRContext gvrContext,
        ...
        for (int col = 0; col<segmentNumber; col++) {
            for (int row = 0; row<segmentNumber; row++) {
                ...
                subMeshes[index] = new SXRMesh(gvrContext, "float3 a_position float2 a_texcoord float3 a_normal");
        ...
        for (int col = 0; col<segmentNumber; col++) {
            for (int row = 0; row<segmentNumber; row++) {
                ...
                subMeshes[index] = new SXRMesh(gvrContext, "float3 a_position float3 a_normal float2 a_texcoord");
                subMeshes[index].setVertices(vertices);
                subMeshes[index].setNormals(normals);
                subMeshes[index].setTexCoords(texCoords);
                subMeshes[index].setIndices(indices);
