...
public class SXRDirectLight extends SXRLight
{
    ...
    public SXRDirectLight(SXRContext gvrContext)
    {
    ...
    public SXRDirectLight(SXRContext ctx, String uniformDesc, String vertexDesc)
    {
     ...
     * The built-in phong shader {@link SXRPhongShader} uses a {@code vec4} uniform named
    ...
    public void setCastShadow(boolean enableFlag)
    {
        ...
        if (owner != null)
        {
            ...
            if (enableFlag)
            {
                ...
                {
                    SXRCamera shadowCam = SXRShadowMap.makeOrthoShadowCamera(
    ...
    public void setShadowRange(float near, float far)
    {
        ...
        if (shadowMap != null)
        {
            shadowCam = (SXROrthogonalCamera) shadowMap.getCamera();
        ...
        {
           shadowCam = SXRShadowMap.makeOrthoShadowCamera(
                    getSXRContext().getMainScene().getMainCameraRig().getCenterCamera());
            shadowCam.setNearClippingDistance(near);
            shadowCam.setFarClippingDistance(far);
            shadowMap = new SXRShadowMap(getSXRContext(), shadowCam);
