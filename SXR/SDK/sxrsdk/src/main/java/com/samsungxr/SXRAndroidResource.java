...
import com.samsungxr.asynchronous.CompressedTexture;
 ...
 * 
 * APIs that load resources can take a {@link SXRAndroidResource} instead of
 ...
 * @since 1.6.1
 */
public class SXRAndroidResource {
    private static final String TAG = Log.tag(SXRAndroidResource.class);
     ...
     */
    public SXRAndroidResource(String path) throws FileNotFoundException {
     ...
     *             File doesn't exist, or can't be read.
     */
    public SXRAndroidResource(File file) throws FileNotFoundException {
     ...
     */
    public SXRAndroidResource(SXRContext gvrContext, int resourceId) {
     ...
     *            A {@code R.raw} or {@code R.drawable} id
     */
    public SXRAndroidResource(Context context, int resourceId) {
     ...
     */
    public SXRAndroidResource(SXRContext gvrContext,
     ...
     *            {@code assets/foo/bar.png}
     */
    public SXRAndroidResource(Context context, String assetRelativeFilename) {
        this.context = context.getApplicationContext();
        streamState = StreamStates.NEW;
     ...
     */
    public SXRAndroidResource(SXRContext context, URL url) {
     ...
     */
    public SXRAndroidResource(String name, InputStream inputStream) {
     ...
     * </ul>
     * @return SXRAndroidResource.ResourceType giving type of resource
     */
     ...
     * {@linkplain SXRAndroidResource.StreamStates#OPEN OPEN}.
     * 
    ...
    public synchronized void openStream() throws IOException {
        ...
        case NETWORK:
            if (!enableUrlLocalCache) {
                ...
                File file = SXRAssetLoader.downloadFile(context,
                        url.toString());
     ...
     * 
     * All {@link SXRAndroidResource} streams support
     ...
     *
     * @return path of the SXRAndroidResource. May return null if the
     ...
     * 
     * @return Filename of the SXRAndroidResource. May return null if the
     *         resource is not associated with any file
     */
    ...
    public boolean equals(Object obj) {
        ...
        SXRAndroidResource other = (SXRAndroidResource) obj;
        switch (resourceType) {
        case ANDROID_ASSETS:
    ...
    public interface Callback<T extends SXRHybridObject> {
         ...
         *            successfully
         */
        void loaded(T resource, SXRAndroidResource androidResource);
         ...
         */
        void failed(Throwable t, SXRAndroidResource androidResource);
    }
     ...
     * moderately complex meshes can be even slower. SXRF uses a throttling
     ...
     * system gets to it. The {@link #stillWanted(SXRAndroidResource)} method
     * lets you cancel unneeded loads.
     */
    public interface CancelableCallback<T extends SXRHybridObject> extends
            Callback<T> {
        /**
         ...
         *         {@link #failed(Throwable, SXRAndroidResource) failed().})
         */
        boolean stillWanted(SXRAndroidResource androidResource);
    }
     ...
     * {@link BitmapTextureCallback#stillWanted(SXRAndroidResource)}
     */
     ...
     * {@link SXRContext#loadTexture(SXRAndroidResource.TextureCallback, SXRAndroidResource)}
     * APIs.
     * 
     * @since 1.6.7
     */
    public interface TextureCallback extends CancelableCallback<SXRImage> {
    }

    /** Callback for asynchronous mesh loads */
    public interface MeshCallback extends CancelableCallback<SXRMesh> {
    }

    private SXRCompressedTextureLoader compressedLoader = null;
    private boolean isCompressedTextureSniffed = false;

    public synchronized SXRCompressedTextureLoader getCompressedLoader() {
