...
public class SXRSpotLight extends SXRPointLight
{
    ...
    public SXRSpotLight(SXRContext gvrContext)
    {
    ...
    public SXRSpotLight(SXRContext gvrContext, String uniformDesc, String vertexDesc)
    {
        ...
        if (owner != null)
        {
            ...
            if (enableFlag)
            {
                ...
                else
                {
                    ...
                    SXRCamera shadowCam = SXRShadowMap.makePerspShadowCamera(getSXRContext().getMainScene().getMainCameraRig().getCenterCamera(), angle);
     ...
     * camera's projection, you can call {@link SXRShadowMap#getCamera}
     * and update them.
     * @param near near shadow camera clipping plane
     * @param far far shadow camera clipping plane
     * @see SXRShadowMap#getCamera()
     */
    public void setShadowRange(float near, float far)
    {
        ...
        if (shadowMap != null)
        {
            shadowCam = (SXRPerspectiveCamera) shadowMap.getCamera();
        ...
        {
            shadowCam = SXRShadowMap.makePerspShadowCamera(
                    getSXRContext().getMainScene().getMainCameraRig().getCenterCamera(), getOuterConeAngle());
            shadowCam.setNearClippingDistance(near);
            shadowCam.setFarClippingDistance(far);
            shadowMap = new SXRShadowMap(getSXRContext(), shadowCam);
            owner.attachComponent(shadowMap);
    ...
    public void onDrawFrame(float frameTime)
    {
        ...
        SXRShadowMap shadowMap = (SXRShadowMap) getComponent(SXRShadowMap.getComponentType());
