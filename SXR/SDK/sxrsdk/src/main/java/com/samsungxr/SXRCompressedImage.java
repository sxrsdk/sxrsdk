...
public class SXRCompressedImage extends SXRImage
{
     ...
     * {@link SXRAssetLoader#loadTexture(SXRAndroidResource, SXRAndroidResource.TextureCallback, SXRTextureParameters, int, int)}
     * 
     * This copy has been 'clamped' to one of the
     * {@linkplain SXRCompressedImage#SPEED public constants} in
     ...
     * @param gvrContext    SXRContext to use for texture.
     ...
     */
    public SXRCompressedImage(SXRContext gvrContext, int width, int height, int imageSize, int format, byte[] data, int levels, int quality)
    {
        super(gvrContext, NativeBitmapImage.constructor(ImageType.BITMAP.Value, format));
        mQuality = SXRCompressedImage.clamp(quality);
        mData = data;
        mWidth = width;
        mHeight = height;
        mLevels = levels;
