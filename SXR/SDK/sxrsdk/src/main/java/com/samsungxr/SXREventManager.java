 ...
 * {@link IScriptEvents#onInit(SXRContext)}, and per-frame callbacks
 ...
 * {@link IScriptEvents#onInit(SXRContext)} is<p>
 *
 * <pre>
 * {@code
...
public class SXREventManager {
    private static final String TAG = SXREventManager.class.getSimpleName();
    ...
    SXREventManager(SXRContext gvrContext) {
     ...
     *    event is delivered to listeners added to the {@link SXREventReceiver} object.
     *    See {@link SXREventReceiver} for more information.
     *    </li>
     * <li> If the target is bound with scripts, the event is delivered to the scripts.
     * A script can be attached to the target using {@link com.samsungxr.script.SXRScriptManager#attachScriptFile(IScriptable, SXRScriptFile)}.</li>
     * </ol>
     *
     * @param target
    ...
    protected boolean sendEventWithMaskParamArray(int sendMask, Object target, Class<? extends IEvents> eventsClass,
        ...
        if ((sendMask & SEND_MASK_LISTENERS) != 0) {
            ...
            if (target instanceof IEventReceiver) {
                ...
                SXREventReceiver receiver = receivingTarget.getEventReceiver();
