...
abstract class SXRConfigurationManager {

    protected WeakReference<SXRApplication> mApplication;
    private boolean isDockListenerRequired = true;
    private boolean mResetFovY;
    private final long mPtr;

    protected SXRConfigurationManager(SXRApplication activity) {
    ...
    protected void addDockListener(SXRApplication activity) {
        activity.addDockListener(new SXRApplication.DockListener() {
            @Override
            public void onDock() {
                handleOnDock();
            }
    ...
    private final Runnable mUsbCheckRunnable = new Runnable() {
        ...
        @Override
        public void run() {
            final SXRApplication application = mApplication.get();
            if (null == application) {
                return;
            }
    ...
    protected void configureForHeadset(final String model) {
        ...
        final float fovY;
        final SXRViewManager viewManager = application.getViewManager();
    ...
    void setFovY(float fovY) {
        ...
        SXRPerspectiveCamera.setDefaultFovY(fovY);

        if (null != viewManager) {
            final SXRCameraRig cameraRig = viewManager.getMainScene().getMainCameraRig();
    ...
    /**
     * For cases where the dock events are not required by the {@link SXRViewManager}, this
    ...
    private void updatePerspectiveCameraFovY(final SXRCamera camera, final float fovY) {
        if (camera instanceof SXRPerspectiveCamera) {
            ((SXRPerspectiveCamera) camera).setFovY(fovY);
        }
    }
