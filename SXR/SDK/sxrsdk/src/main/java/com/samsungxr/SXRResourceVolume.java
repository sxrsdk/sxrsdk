...
public class SXRResourceVolume {
    private static final String TAG = SXRResourceVolume.class.getSimpleName();
    ...
    /**
     * Constructor. Creates a {@link SXRResourceVolume} object based on a volume type.
     * @param gvrContext The SXR Context.
     * @param volume The volume type.
     */
    public SXRResourceVolume(SXRContext gvrContext, VolumeType volume) {
        this(gvrContext, volume, null);
    }

    /**
     * Constructor. Creates a {@link SXRResourceVolume} object based on a resource.
     *
     ...
     *
     * If a volume is opened using a SXRAndroidResource that references a file
     ...
     *
     * @param context The SXR Context.
     * @param resource SXRAndroidResource residing on the volume. This resource is added
     *                 to the internal resource map of this volume so it can be opened
     *                 based on only the filename using SXRResourceVolume.openResource.
     * @see #getFileName()
     */
    public SXRResourceVolume(SXRContext context, SXRAndroidResource resource)
    {
    ...
    /**
     * Constructor. Creates a {@link SXRResourceVolume} object based on a filename.
     *
     ...
     * @see #getFileName()
     */
    public SXRResourceVolume(SXRContext context, String filename)
    {
        String fname = filename.toLowerCase();
        gvrContext = context;
        volumeType = SXRResourceVolume.VolumeType.ANDROID_ASSETS;
     ...
     * volume.
     */
    public SXRResourceVolume(SXRContext gvrContext, VolumeType volumeType, String defaultPath) {
     ...
     */
    public SXRResourceVolume(SXRContext gvrContext,
    ...
    private ConcurrentHashMap<String, SXRAndroidResource> resourceMap = new ConcurrentHashMap<String, SXRAndroidResource>();

    /**
     * Adds an android resource to the volume's resource map.
     * This function is useful if you have created the SXRAndroidResource from a resource ID
     ...
     * @throws IOException
     */
    public SXRAndroidResource addResource(SXRAndroidResource resource)
    {
        String fileName = resource.getResourcePath();
        SXRAndroidResource resourceValue = resourceMap.get(fileName);
    ...
    public SXRAndroidResource openResource(String filePath) throws IOException {
        ...
        SXRAndroidResource resourceKey;
        switch (volumeType) {
        case ANDROID_ASSETS:
            // Resolve '..' and '.'
            ...
            resourceKey = new SXRAndroidResource(gvrContext, path);
            break;
            ...
            resourceKey = new SXRAndroidResource(gvrContext, resourceId);
            break;
        ...
        case INPUT_STREAM:
            resourceKey = new SXRAndroidResource(getFullPath(defaultPath, filePath), volumeInputStream);
            break;

        case NETWORK:
            resourceKey = new SXRAndroidResource(gvrContext,
     ...
     * This filename is only available if the SXRResourceVolume was constructed
     * with the SXRResourceVolume(SXRContext, String) constructor.
     ...
     * @return File path compatible with the file system of this {@link SXRResourceVolume}.
     */
