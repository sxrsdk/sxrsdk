...
MainSceneSorter::MainSceneSorter(int numMatrices, bool forceTransformBlock)
: RenderSorter("MainSorter", numMatrices, forceTransformBlock)
{
...
void MainSceneSorter::add(RenderState& rstate, Node* object, Renderer& renderer)
{
...
#ifdef DEBUG_RENDER
        Node* owner = r->renderData->owner_object();
...
void MainSceneSorter::validate(RenderState& rstate, Renderer& renderer)
{
    rstate.shadow_map = rstate.scene->getLights().updateLights(&renderer);
    RenderSorter::validate(rstate, renderer);
}

bool MainSceneSorter::isValid(RenderState& rstate, Renderable& r, Renderer& renderer)
{
    if (r.shader)
    {
    ...
    return RenderSorter::isValid(rstate, r, renderer);
}
