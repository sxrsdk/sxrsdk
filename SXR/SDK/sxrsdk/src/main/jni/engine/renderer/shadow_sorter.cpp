...
bool ShadowRenderSorter::isValid(RenderState& rstate, Renderable& r, Renderer& renderer)
{
...
Shader* ShadowRenderSorter::selectShader(const RenderState& rstate, Renderable& r, Renderer& renderer)
{
    ...
    const char* depthShaderName = index ? "SXRDepthShader$a_bone_weights$a_bone_indices" : "SXRDepthShader";
    ...
    #endif
        if (rstate.scene->makeDepthShaders(&renderer, rstate.javaSceneObject))
        {
            shader = rstate.shader_manager->findShader(depthShaderName);
            if (shader == nullptr)
            {
